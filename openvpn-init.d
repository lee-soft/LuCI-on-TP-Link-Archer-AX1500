...
section_enabled() {
    local wanproto=$(uci get network.wan.wan_type 2>/dev/null)
...
start_instance() {
	...
	# only append server access rules for non-client instances
	local is_client=0
    config_get_bool is_client "$s" "client" 0
    [ "$is_client" = "0" ] && append_access "$s"
    # if a config file is specified, use it directly instead of generated conf
    local ovpn_config=
    config_get ovpn_config "$s" "config"
    SERVICE_PID_FILE="/var/run/openvpn-$s.pid"
    rm -f "$SERVICE_PID_FILE"
    if [ -n "$ovpn_config" ] && [ -f "$ovpn_config" ]; then
        local auth_file=
        config_get auth_file "$s" "auth_user_pass"
        local auth_arg=""
        [ -n "$auth_file" ] && auth_arg="--auth-user-pass $auth_file"
        /usr/sbin/openvpn --syslog "openvpn($s)" --writepid "$SERVICE_PID_FILE" --config "$ovpn_config" $auth_arg --daemon
    else
        /usr/sbin/openvpn --syslog "openvpn($s)" --writepid "$SERVICE_PID_FILE" --config "/var/etc/openvpn-$s.conf" --daemon
    fi
}
